...
import numpy as np
DATA_DIR = 'data/'
data = np.loadtxt(DATA_DIR + 'bank/train.csv', delimiter=',', dtype = str)
...
#     string=string_[:]
#     if method=='entropy':
#         ntropy_diffind,entropy_diffmax=Entroy_tree(data_obj)
#         #print("*****",ntropy_diffind,"is the selected attribute","maximum information gain is",entropy_diffmax,method)

#     elif method=='Gini':
#         ntropy_diffind,entropy_diffmax=Gini_tree(data_obj)
#         #print("*****",ntropy_diffind,"is the selected attribute","maximum information gain is",entropy_diffmax,method)
#     #print(ntropy_diffind)
#     elif method=='MajorityError':
#         ntropy_diffind,entropy_diffmax=MajorityError_tree(data_obj)
#     else:
#         raise 
#     tag_num=data_obj.raw_data.shape[0]
...
#                 continue
#             if num_iter1>=maxi_iter:
#                 tag_num=data_subset.raw_data.shape[0]
                
#                 values, counts = np.unique(data_obj.raw_data[:,LabelDim], return_counts=True)
#                 most_common_sub=values[np.argmax(counts)]
#                 #string_2=string_1+str(ntropy_diffind)+"->"+str(i)+"->"+str(most_common_sub)
#                 string_2=string_1+[ntropy_diffind]+[i]+["label"]+[most_common_sub]
#                 tree_decision.append(string_2)
#                 #print(string_2)
#                 continue
#             #string_2=string_1+str(ntropy_diffind)+"->"+str(i)+"->"
#             string_2=string_1+[ntropy_diffind]+[i]
#             #print(string_2,num_iter1)
#             tree_decision=itera_tree(data_subset,string_2,num_iter1,maxi_iter,tree_decision=tree_decision)
#     return    tree_decision
...
def batch_predic(data_obj2,tree_dec_np_entro):
...
def train_predict(data_obj,data_obj2,depth_limit=1,method='entropy'):
    ...
    tree_dec_np=conver2numpy(tree_dec)
    return batch_predic(data_obj,tree_dec_np),batch_predic(data_obj2,tree_dec_np)
...
print("*"*50,"Car")
depth_option=np.array([1,2, 3, 4, 5, 6])
methods=["entropy","Gini","MajorityError"]
# methods=["MajorityError"]
for i in range(depth_option.shape[0]):
    trainRes=[]
    testRes=[]
...
print("*"*50,"Bank while treat unknown as feature")

DATA_DIR = 'data/'
data_train = Data(fpath = DATA_DIR + 'bank/train.csv')
data_test = Data(fpath = DATA_DIR + 'bank/test.csv')

##binarize feature according to median.
numericDimList=[0,5,9,11,12,13,14]
...
LabelDim=-1
depth_option=[1,2,3,4,5,6,7,8,10,13,16]
methods=["entropy","Gini","MajorityError"]
